        ...
        try:
            ...
            while current_turn < max_turns:
                ...
                if message.content and message.content.strip() != "":
                    broadcast_log("ai", message.content, f"FeedbackAgent-R{current_turn}:{message.content}")
                
                # 检查是否有工具调用
                if hasattr(message, 'tool_calls') and message.tool_calls:
                    # 处理工具调用
                    tool_message = (f"调用工具-{message.tool_calls[0].function.name}-参数-{message.tool_calls[0].function.arguments}")
